...
Join with box set again for the geometry and map.
```{r}
atlantis_ssl_winter <- atlantis_box %>% select(.bx0,area,boundary) %>% left_join(abundance_by_box_winter, by = '.bx0') %>%
  mutate(log_abundance = log1p(abundance),
         density = abundance/area*1000000, # back to km2
         logdensity = log1p(density)) 
...
Write out summer and winter distributions.
```{r}
s2_s3 <- atlantis_ssl %>% 
  select(.bx0,abundance) %>% 
  st_set_geometry(NULL) %>% 
...
# write out
write.csv(s2_s3, 'ssl_s2_s3.csv', row.names = F)
write.csv(s1_s4, 'ssl_s1_s4.csv', row.names = F)
```
