...
The range around the rookeries will be smaller than around the haulouts. To make matters more complicated, a rookery is also a haulout, but not *vice versa* (I think). Therefore, rookery sites also count towards winter distributions. It also seems possible that some sites are used as haulouts at one point and as rookeries later on, or *vice versa* (for example where no pups were counted until a certain year and then they appear more or less regularly).
...
missing1 <- rbind(missing_nonpup1,missing_pup1) %>%
  mutate(LONGITUDE = c(-156.639,-149.467,-130.616,-130.917,-130.967),
         LATITUDE = c(56.044,59.742,53.490,51.962,52.208)) 
...
```{r}
nonpup_sf <- nonpup_sf %>% filter(YEAR %in% (1990:1999)) # use the 90's
...
# view
nonpup_avg %>% st_transform(crs=atlantis_crs) %>% 
  ggplot()+
  geom_sf(data=coast_sf)+
...
# Circles around colonies

Incorporate the different dispersal from rookery vs haulout here.
```{r}
...
colony_at_sea %>% ggplot()+
  geom_sf(aes(fill=log1p(COUNTS)),alpha=0.5)+
  scale_fill_viridis()+
  geom_sf(data=coast_sf)+
  coord_sf(xlim=c(atlantis_bbox$xmin, atlantis_bbox$xmax),ylim=c(atlantis_bbox$ymin, atlantis_bbox$ymax+50000))+
  theme_minimal()+
  labs(title='Steller sea lions',fill='Log-counts')
```
...
```{r, warning=FALSE}
# shed some variables
box_thin <- atlantis_box %>% select(.bx0,boundary)

# calculate the area of the intersection with each atlantis box in the model, and scale the n of ind for that colony by the area of the intersection
...
```{r}
atlantis_ssl <- atlantis_box %>% select(.bx0,area,boundary) %>% left_join(abundance_by_box, by = '.bx0')
...
atlantis_ssl %>% 
  ...
  ungroup() %>%
  ggplot()+
  geom_sf(aes(fill=log1p(as.numeric(abundance/(area/1000000)))))+ # back to km2
  scale_fill_viridis()+
  ...
  theme_bw()+
  labs(title='Steller sea lions in Atlantis GOA', fill = expression(paste('Logdensity (ind ',km^-2, ')')))

#ggsave(paste0(groups[4],'.png'), p, dpi=600, width=10, height=4)
```

Results depend on the foraging range. Need to do some reading on that - check the papers Brian shared on feeding. 